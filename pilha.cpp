...
TipoCelulaPilha *Topo, *Topo2; //Variável utilizada para manter o controle do estado atual da pilha
                  //Como a pilha é implementada em memória dinâmica,
                  //Topo deve ser um ponteiro para a célula no topo da Pilha

int ContadorPilha, ContadorPilha2 ;  //Variável para contar o número de elementos armazenados na Pilha
...
void Pilha_Construtor2() // pilha auxiliar para fazer a ordenação das notas
{

  Topo2 = NULL;  //O construtor da Pilha deve iniciar uma Pilha vazia, sem memória alocada
                //para essa estrutura de dados (não há ainda elementos armazenados)
                //Portanto, o ponteiro Topo deve apontar para NULL

  ContadorPilha2 = 0;  //Inicia a variável para indicar a quantidade de elementos armazenados na Pilha
...
void Pilha_Destrutor2()
{
   ...
   {
     Temp = Topo2;  //o ponteiro temporário deve apontar para o mesmo endereço que Topo
     Topo2 = Topo2 -> Prox; //Topo avança para a célula seguinte
...
bool Pilha_Vazia2()
{
   return Topo2 == NULL;   //Pela forma como os elementos são Empilhados e Desempilhados (métodos a seguir)
                          //a Pilha está vazia se Topo apontar para NULL
...
int Pilha_Tamanho2()
{
    ...
    return ContadorPilha2;   //É retornado o contador de elementos, manipulado nos métodos de Empilhar e Desempilhar
}
...
bool Pilha_Empilhar(TipoDadoPilha Elemento)
{
   ...
   if((Nova = (TipoCelulaPilha *) malloc(sizeof(TipoCelulaPilha)))==NULL)  //tenta alocar dinâmicamente na memória umam nova célula da Pilha
   ...
   else
   {
       ...
       //cout << "Entrou na pilha -> " << Nova->Item << "\n";
...
bool Pilha_Empilhar2(TipoDadoPilha Elemento)
{
   ...
   {
       Nova->Item = Elemento;  //Como Nova aponta para um espaço de memória disponível, guardamos o Elemento a ser armazenado nesse espaço
       Nova->Prox = Topo2;      //Para fazer a ligação entre as células da Pilha, o novo espaço de memória reservado
                               //e apontado por Nova possui um ponteiro Prox que deve apontar para célua seguinte
                               //Nesse caso, o ponteiro Prox deve apontar para o mesmo endereço que Topo aponta
                               //Resumindo: O Prox do Nova aponta agora para Topo

       Topo2 = Nova;            //Como a Pilha possui agora um novo topo apontado por Nova e o ponteiro Topo está um nível abaixo
                            //deve-se fazer com que Topo aponte para a nova célula criada

       ContadorPilha2++;             //Como foi armazenado um novo Elemento, o contador deve ser incrementado
       //cout << "Entrou na pilha -> " << Nova->Item << "\n";
       return true;            //Retorna verdadeiro, indicando que o elemento foi empilhado (inserido na Pilha)
...
bool Pilha_Desempilhar2(TipoDadoPilha &Elemento)
{
   ...
   if(!Pilha_Vazia())  //primeiro deve-se conferir se a Pilha não está vazia
   {
      ...
      TipoCelulaPilha *Temp = Topo2;  //Cria um ponteiro temporário (Temp) e faz apontar para o mesmo endereço de Topo
                                //Essa célula será excluída da Pilha

      Topo2 = Topo2->Prox; //Faz o topo apontar para a célula seguinte
                         //Esse procedimento define a célula eleita como topo da Pilha
      ...
      ContadorPilha2--;          //Como foi removido um Elemento, o contador deve ser decrementado
...
bool Pilha_Topo2(TipoDadoPilha &Elemento)
{
     ...
     if(!Pilha_Vazia())  //primeiro deve-se conferir se a Pilha não está vazia
     {
        Elemento = Topo2->Item;  //Topo aponta uma célula com o último elemento empilhado
